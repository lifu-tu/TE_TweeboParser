...
rm -f -r ${WORKING_DIR}/test_score1
mkdir ${WORKING_DIR}/test_score1

# --> Parse the first time using token embedding model to get the scores
cp ${WORKING_DIR}/test ${TE_DIR}/test

cd ${TE_DIR}
python DNN_TE_Score.py

cd ${PARSER_DIR}
# --> Parse the second time using PTB model to get the scores
...
# --> Parse the third time using PTB score as features to get the final results
./TurboParser --test --file_model=${MODEL_DIR}/parsing_model2 --file_test=${WORKING_DIR}/test --file_prediction=${WORKING_DIR}/test_predict --output_posterior=false --use_posterior=true --posterior_dir=${WORKING_DIR}/test_score
