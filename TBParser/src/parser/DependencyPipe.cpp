...
void DependencyPipe::MakeSelectedFeatures(Instance *instance,
  ...
  vector<int> h_vector1;
  vector<int> m_vector1;
  vector<double> value_vector1;
  ...
  if (options->use_posterior()) {
    ...
    string path1 = options->GetPosteriorDir() + "1/Token_" + indexs ;
    ifstream in1(path1.c_str());
    string s1;
    ...
    while(getline(in1,s1))
    {
      ...
      str[s1.size()] = 0;
      memcpy(str, s1.c_str(), s1.size());

      int h1;
      int m1;
      double v1 = 0.0;
      sscanf(str, "%d\t%d\t%lf", &h1, &m1, &v1);
      //if (v >=  0.1){
      h_vector1.push_back(h1);
      m_vector1.push_back(m1);
      value_vector1.push_back(v1);
	//}
      //LOG(INFO)<< h << "\t" << m << "\t" << v << endl;
    }
    /*
  ...
  for (int r = offset; r < offset + size; ++r) {
    ...
    if (options->use_posterior()) {
     ...
     double value1 = FindingValue(arc->head(), arc->modifier(), h_vector1,
          m_vector1, value_vector1);

      for (uint16_t i = 0; i < 200; i++) {
        uint8_t fired = value1 * 200  > i  ? 1 : 0;
        //uint8_t fired = value1 * 10 + 100 > i ? 1 : 0;
        if(fired == 0){
          break;
        }
        dependency_features->AddBaseTokenFeatures(r, fired, i);
      }
      //if(value < 0){
      //  dependency_features->AddBasePTBFeatures(r, 1, 101);
      //}else{
       // dependency_features->AddBasePTBFeatures(r, 0, 101);
      //}
    }
  }
